...
def get_cost_kernel(M, N, K, K_sub=None):
    if not K_sub:
        K_sub = K
    ...
    if M >= N:
        assert N * K_sub * AB_element_size < L2_size, f"when loop N first, B should fit in L2 to fully reuse, problem {N=} {K_sub=}"
    else:
        assert M * K_sub * AB_element_size < L2_size, f"when loop M first, A should fit in L2 to fully reuse, problem {M=} {K_sub=}"
...
shapes = [
    ...
    {"B": 8, "M": 1024, "N" : 1024, "K": 2048},
    {"B": 8, "M": 1024, "N" : 1024, "K": 3072},
...
for shape in shapes:
    get_cost(shape['B'], shape['M'], shape['N'], shape['K'])
...
# ]
# for shape in shapes:
#     get_cost_kernel(shape['M'], shape['N'], shape['K'])
